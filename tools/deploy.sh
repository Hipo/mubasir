...
# GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_backend" git reset --hard
GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_backend" git fetch
GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_backend" git checkout production
GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_backend" git pull origin production
...
CONTAINER_UID=$(id -u) CONTAINER_GID=$(id -g) docker-compose -f docker-compose.yml -f docker-compose.production.yml run mubasir ./manage.py migrate --no-input
CONTAINER_UID=$(id -u) CONTAINER_GID=$(id -g) docker-compose -f docker-compose.yml -f docker-compose.production.yml run mubasir ./manage.py collectstatic --no-input
CONTAINER_UID=$(id -u) CONTAINER_GID=$(id -g) docker-compose -f docker-compose.yml -f docker-compose.production.yml start mubasir
