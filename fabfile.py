...
def deploy_app(branch, deploying_to_first_server, with_smoke_test):
    ...
    with cd(directory):
        run('GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_github_backend_id_rsa" git reset --hard')
        run('GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_github_backend_id_rsa" git fetch')
        run('GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_github_backend_id_rsa" git checkout %s' % branch)
        run('GIT_SSH_COMMAND="ssh -i ~/.ssh/mubasir_github_backend_id_rsa" git pull origin %s' % branch)
        ...
        if deploying_to_first_server == 'yes':
            sudo('{docker_cmd} run mubasir ./manage.py migrate --no-input'.format(docker_cmd=docker_cmd))

        # Start
        sudo('{docker_cmd} start mubasir rapid7 notification_sender'.format(docker_cmd=docker_cmd))

        sudo("ln -sfn /mubasir/conf/rapid7_{branch}_rsyslog.conf /etc/rsyslog.d/rapid7.conf".format(branch=branch))
        ...
        if with_smoke_test == "yes":
            ...
            try:
                ...
                sudo('{docker_cmd} stop mubasir'.format(docker_cmd=docker_cmd))
                raise
...
def collectstatic(branch):
    ...
    with cd(directory):
        while keep_trying:
            try:
                sudo('{docker_cmd} run mubasir ./manage.py collectstatic --no-input'.format(docker_cmd=docker_cmd))
                keep_trying = False
